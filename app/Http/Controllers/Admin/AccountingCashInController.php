...
class AccountingCashInController extends Controller
{
    ...
    {
        $this->middleware('permission:allAccountingCashIn', ['only' => ['index']]);
        $this->middleware('permission:addAccountingCashIn', ['only' => ['create', 'store']]);
...
//            dd($request->images);
        ...
        try {
            ...
            if ($safe->type == "safe") {
                ...
                $message ='New Accounting CashIn # '. $row->id .' Added by ('.auth()->user()->name.')';
...
//        $client->roles()->sync($request->input('role'));
            ...
            return redirect('admin/accounting-cash-in');
