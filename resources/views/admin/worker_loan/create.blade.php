@extends('admin.layouts.main')

@section('breadcrumb')

    @include('admin.layouts.partials.breadcrumb',['title'=>trans('main.worker_loans'),'url'=>'worker-loan'])
@stop
@section('content')
    <!-- FILE: app/views/start.blade.php -->
    <div class="ibox ">
        <div class="ibox-title">
            <h5>{{trans('main.worker_loans')}} |  {{trans('main.new')}} </h5>
            <div class="ibox-tools">
                <a class="collapse-link">
                    <i class="fa fa-chevron-up"></i>
                </a>
                <a class="close-link">
                    <i class="fa fa-times"></i>
                </a>
            </div>
        </div>
        <!-- form start -->
        {!! Form::model($model,[
                             'action'=>'Admin\WorkerLoanController@store',
                             'id'=>'myForm',
                             'role'=>'form',
                               'files'=>'true',
                             'method'=>'POST'
                             ])!!}
        <div class="ibox-content">
            @include('admin.worker_loan.form')
        </div>
        <div class="ibox-footer">


            <div class="row">
                <div class="col-sm-6 col-sm-push-2">
                    <button type="submit" class="btn btn-primary disableOnSubmit">{{trans('main.save')}}</button>
                </div>
                <div class="col-sm-6">
                    <button type="reset" class="btn btn-danger">{{trans('main.cancel')}}</button>
                </div>
            </div>
        </div>
        {!! Form::close()!!}

    </div>

    @push('script')
        <script>
            $(document).on('change', '#worker_ids', function (e) {
                var x= $("#worker_ids :selected").map(function(i, el) {
                    return $(el).text();
                }).get();
                x= x.sort();
                var html = '';
                var counter = 1;
                $.each(x, function(key, value){
                    html += `
                 <tr class="sec-content-row" itemid="${counter}" >
                    <td>${counter}</td>
                    <td>${value}</td>
                </tr>
                        `;
                    counter++;
                });
                $(".item-append-area").html(html);
            });
        </script>
    @endpush
@stop
