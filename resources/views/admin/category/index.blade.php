...
@section('breadcrumb')
    ...
    @include('admin.layouts.partials.breadcrumb',['title'=>trans('main.categories'),'url'=>'category'])
@stop
@section('content')
            ...
            <div class="row">
                <div class="col-sm-4">
                    <div class="form-group">
                        <label class="control-label" for="main_category">{{trans('main.main_category') }}</label>
                        {{Form::select('main_category', $categories, request()->main_category, [
                              "class" => "form-control select2 " ,
                              "id" => "main_category",
                              "placeholder" => trans('main.select_main_category')
                          ])}}
                    </div>
                </div>
                <div class="col-sm-4">
            ...
            @if($rows->count()>0)
                <div class="table-responsive">
                    <table class="data-table table table-bordered">
                        ...
                        <th>{{trans('main.main_category') }}</th>
                        @can('editCategory')
                        ...
                        @foreach($rows as $row)
                            ...
                            <tr>
                                <td>{{$iteration}}</td>
                                <td>{{$row->name}}</td>
                                <td>{{$row->main_display}}</td>
                                @can('editCategory')
                                    <td class="text-center"><a href="{{url('admin/category/'.$row->id.'/edit')}}"
                                                               class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a>
                                    </td>
                                @endcan
                                @can('deleteCategory')
                                    <td class="text-center">
                                        {{Form::open(array('method'=>'delete','class'=>'delete','url'=>url('admin/category/'.$row->id) )) }}
