...
@section('content')
    ...
    @can('addEmpApp')
        <div class="">
            <a href="{{url('admin/applicant-employee/create')}}" class="btn btn-primary">
                ...
                @foreach($rows as $row)
                    <div class="col-lg-6">
                        <div class="contact-box">
                            <a href="{{url('admin/applicant-employee/'.$row->id)}}">
                                <div class="col-sm-4">
                                    <div class="text-center">
                                        ...
                                        {{--                                        {{dd($row->job->name)}}--}}
                                        <div class="m-t-xs font-bold"><span
                                                    class="label label-primary">{{$row->evaluation_status_display ?? ''}}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-8">
                                    <h3><strong>{{$row->name}}</strong></h3>
                                    <p><i class="fa fa-globe"></i> {{$row->country->nationality ?? ''}}</p>
                                    <address>
                                        <strong> <i class="fa fa-briefcase"></i> {{$row->job->name  ?? ''}}
                                        </strong><br>
